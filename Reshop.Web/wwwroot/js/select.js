...
function SelectDropdown(options){
  ...
  document.querySelectorAll("select:not([multiple])").forEach((el,k)=>{
    ...
    var div=newEl('div',{class:'select-dropdown'});
    el.style.display='none';
    el.parentNode.insertBefore(div,el.nextSibling);
    var listWrap=newEl('div',{class:'select-dropdown-list-wrapper'});
    var list=newEl('div',{class:'select-dropdown-list'});
    var search=newEl('input',{class:['select-dropdown-search'],style:{display:el.attributes['select-search']?.value==='true'?'block':'none'},placeholder:'جستجو'});
    listWrap.appendChild(search);
    div.appendChild(listWrap);
    listWrap.appendChild(list);
...
window.addEventListener('load',()=>{
  SelectDropdown(window.SelectDropdownOptions);
});
